...
# Configuration
project_name = "rag-automation" # at least 3 characters
region = "us-west-2"
git_name = "rag-automation" # BDA
...
# BDA CRIS: data-automation-profile ARNs per source region (README §7)
_BDA_CRIS_REGIONS_BY_SOURCE = {
...
_BDA_PROFILE_SUFFIX_BY_SOURCE = {
    "us-east-1": "us.data-automation-v1",
    "us-west-2": "us.data-automation-v1",
    "eu-central-1": "eu.data-automation-v1",
    "eu-west-1": "eu.data-automation-v1",
    "eu-west-2": "eu.data-automation-v1",
    "ap-south-1": "apac.data-automation-v1",
    "ap-southeast-2": "apac.data-automation-v1",
}


def get_bda_cris_profile_arns(source_region: str) -> List[str]:
    """Return BDA data-automation-profile ARNs for Cross-Region Inference (README §7–8)."""
    profile_suffix = _BDA_PROFILE_SUFFIX_BY_SOURCE.get(source_region, "us.data-automation-v1")
    cris_regions = _BDA_CRIS_REGIONS_BY_SOURCE.get(source_region, [source_region])
    return [
        f"arn:aws:bedrock:{r}:{account_id}:data-automation-profile/{profile_suffix}"
        for r in cris_regions
    ]
        ...
        "Statement": [
            {
                ...
                "Resource": get_bda_cris_profile_arns(region),
            }
        ],
    }
    attach_inline_policy(role_name, f"bda-parser-policy-for-{project_name}", bda_policy)
...
def get_supplemental_data_storage_configuration(s3_bucket_name: str) -> Dict:
    """S3 supplemental storage required for BDA parsing (README §6)."""
    return {
        "storageLocations": [
            {
                "type": "S3",
...
def kb_has_supplemental_storage(kb_details: Dict) -> bool:
    vector_config = (
        kb_details.get("knowledgeBase", {})
        .get("knowledgeBaseConfiguration", {})
        .get("vectorKnowledgeBaseConfiguration", {})
    )
    supplemental = vector_config.get("supplementalDataStorageConfiguration", {})
    return bool(supplemental.get("storageLocations"))


def ensure_data_source_with_bda_parser(
...
def create_knowledge_base_with_opensearch(opensearch_info: Dict[str, str], knowledge_base_role_arn: str, s3_bucket_name: str) -> str:
        ...
        for kb in kb_list.get("knowledgeBaseSummaries", []):
            if kb["name"] == project_name:
                ...
                if kb_collection_arn != opensearch_info["arn"]:
                    ...
                    break
                if not kb_has_supplemental_storage(kb_details):
                    logger.warning(
                        "Knowledge Base missing supplementalDataStorageConfiguration "
                        "(required for BDA parser), recreating..."
                    )
                    delete_knowledge_base(kb["knowledgeBaseId"])
                    break
                ...
                ensure_data_source_with_bda_parser(
    ...
    response = bedrock_agent_client.create_knowledge_base(
        name=project_name,
        description="Knowledge base with BDA parser (OpenSearch Serverless)",
        ...
        knowledgeBaseConfiguration={
            ...
            "vectorKnowledgeBaseConfiguration": {
                ...
                "supplementalDataStorageConfiguration": get_supplemental_data_storage_configuration(
                    s3_bucket_name
                ),
            }
        },
