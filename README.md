...
BDA는 크로스 리전 추론(Cross-Region Inference)을 반드시 사용해야 합니다. 이를 통해 요청된 지리적 경계 내에서 최적의 리전을 자동으로 선택하여 처리합니다. 여기서는 Knowledge Base를 사용하므로 아래의 모델은 사용 리전에 따라 자동으로 선택됩니다. 데이터는 원본 소스 리전에만 저장되며, 전송 중에는 AWS의 암호화된 보안 네트워크를 통해 처리됩니다.
